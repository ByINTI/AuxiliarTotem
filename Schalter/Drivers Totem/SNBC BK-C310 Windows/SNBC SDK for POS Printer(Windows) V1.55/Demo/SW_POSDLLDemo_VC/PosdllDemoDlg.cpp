...
void CPosdllDemoDlg::SetCtrlLang(int ID)
{
	...
	{
	case CHINESE_SIMPLE:
		SetDlgItemText(IDC_STC_PORT, "端口选择");
		SetDlgItemText(IDC_PORT_COM, "串口");
		SetDlgItemText(IDC_PORT_LPT, "并口");
		SetDlgItemText(IDC_PORT_USB, "USB口");
		SetDlgItemText(IDC_PORT_NET, "网络接口");
		SetDlgItemText(IDC_PORT_DRV, "驱动程序");
		SetDlgItemText(IDC_PORT_CLASS, "USB Class");

		SetDlgItemText(IDC_STC_SET, "端口配置");
		SetDlgItemText(IDC_STC_NAME, "串口名称：");
		SetDlgItemText(IDC_STC_RATE, "每秒位数：");
		SetDlgItemText(IDC_STC_DATA, "数据位：");
		SetDlgItemText(IDC_STC_PARITY, "校验位：");
		SetDlgItemText(IDC_STC_STOP, "停止位：");
		SetDlgItemText(IDC_STC_SHAKE, "数据流控制：");
		SetDlgItemText(IDC_STC_PRANAME, "并口名称：");
		SetDlgItemText(IDC_STC_IP, "IP地址：");
		SetDlgItemText(IDC_STC_DRVNAME, "驱动名称：");

		SetDlgItemText(IDC_SAVE_TO_TXT, "数据保存到文件Test.txt，不向端口下发");

		SetDlgItemText(IDC_OPEN_PORT, "打开端口");
		SetDlgItemText(IDC_QUERY_STATUS, "查询状态");
		SetDlgItemText(IDC_PRINT, "打印");
		SetDlgItemText(IDC_CLOSE_PORT, "关闭端口");

		SetDlgItemText(IDC_STC_Sample, "示例设置");
		SetDlgItemText(IDC_STC_MODE, "模式选择");
		SetDlgItemText(IDC_MODE_STANDARD, "标准模式");
		SetDlgItemText(IDC_MODE_PAGE, "页模式");
		SetDlgItemText(IDC_STC_WIDTH, "页宽：");

		m_ctrlParity.AddString("偶校验");
		m_ctrlParity.AddString("奇校验");
		m_ctrlParity.AddString("无校验");

		m_ctrlFlowControl.AddString("XON/OFF");
		m_ctrlFlowControl.AddString("硬件");
		m_ctrlFlowControl.AddString("无");
		break;
	default:
